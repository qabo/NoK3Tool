...
class procedure TfrmMakeFGReport2.ShowForm;
var
  frmMakeFGReport: TfrmMakeFGReport2;
begin
  frmMakeFGReport := TfrmMakeFGReport2.Create(nil);
  try
...
procedure TfrmMakeFGReport2.tbProjNameNoClick(Sender: TObject);
begin
  TfrmProjNameNo.ShowForm;
end;

procedure TfrmMakeFGReport2.btnExitClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmMakeFGReport2.FormCreate(Sender: TObject);
var
  ini: TIniFile;
  s: string;
begin
  ...
  try
    ...
    rgMZ.ItemIndex := ini.ReadInteger(self.ClassName, rgMZ.Name, 0);
  finally
    ini.Free;
  end;
end;

procedure TfrmMakeFGReport2.FormDestroy(Sender: TObject);
var
  ini: TIniFile;
  s: string;
begin
  ...
  try
    ...
    ini.WriteInteger(self.ClassName, rgMZ.Name, rgMZ.ItemIndex);
  finally
    ini.Free;
  end;
end;

procedure TfrmMakeFGReport2.btnWinsClick(Sender: TObject);
var
  sfile: string;
begin
...
procedure TfrmMakeFGReport2.btnStocksClick(Sender: TObject);
var
  sfile: string;
begin
  if not ExcelOpenDialogs(sfile) then Exit;
...
procedure TfrmMakeFGReport2.btnUnOutClick(Sender: TObject);
var
  sfile: string;
begin
  if not ExcelOpenDialogs(sfile) then Exit;
...
procedure TfrmMakeFGReport2.btnDBClick(Sender: TObject);
var
  sfile: string;
begin
  if not ExcelOpenDialogs(sfile) then Exit;
...
procedure TfrmMakeFGReport2.btnSFClick(Sender: TObject);
var
  sfile: string;
begin
  if not ExcelOpenDialogs(sfile) then Exit;
...
procedure TfrmMakeFGReport2.btnKJClick(Sender: TObject);
var
  sfile: string;
begin
  if not ExcelOpenDialogs(sfile) then Exit;
...
procedure TfrmMakeFGReport2.btnORTClick(Sender: TObject);
var
  sfile: string;
begin
  if not ExcelOpenDialogs(sfile) then Exit;
...
procedure SaveWin(ExcelApp: Variant; aFGRptWinReader: TFGRptWinReader;
  slWin, slWinBS, slProjNo: TStringList);
var
...
begin
  ...
  begin          
    if slProjNo.IndexOfName( Copy(aFGRptWinReader.Items[j].snumber, 1, 5) ) < 0 then
    begin
      Continue;
    end;
  ...
  begin
    ...
    begin                                                 
      ...
      if slProjNo.IndexOfName( Copy(aFGWinReader.Items[j].snumber, 1, 5) ) < 0 then
      begin
        Continue;
      end;
...
procedure SaveStock(ExcelApp: Variant; slStock, slStockBS, slDB, slDBBS,
...
begin
  ...
  begin
    ...
    begin
      if slProjNo.IndexOfName( Copy(aFGStockRptReader.Items[j].snumber, 1, 5) ) < 0 then
      begin
        Continue;
      end;
...
procedure SaveUnOut(ExcelApp: Variant; slUnOut, slUnOutBS, slProjNo: TStringList);
var
...
begin
  ...
  begin
    ...
    begin             
      if slProjNo.IndexOfName( Copy(aFGUnOutRptReader.Items[j].snumber, 1, 5) ) < 0 then
      begin
        Continue;
      end;
...
procedure Save_SF_KJ_ORT(ExcelApp: Variant; sl, slProjNo: TStringList);
var 
  irow: Integer;
  j: Integer;
  iino: Integer;
...
function GetSumStockAlll(const sProjNo: string; slStock, slDB: TStringList;
  var dOHOverSea, dOHDomestic: Double): Double;
var
  aFGStockRptReader: TFGStockRptReader;
  i, j: Integer;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if IsNameHW(aFGStockRptReader.Items[j]^.snumber, aFGStockRptReader.Items[j]^.sname) then
        begin
          dOHOverSea := dOHOverSea + aFGStockRptReader.Items[j].dqty;
        end
        else
        begin
          dOHDomestic := dOHDomestic + aFGStockRptReader.Items[j].dqty;
        end;
      end;
    end;
  end;
...
function GetSumSF_KJ_ORT(const sProjNo: string; sl: TStringList;
  var dOHOverSeaSF, dOHDomesticSF: Double; bSF: Boolean): Double;
var
  j: Integer;
  aFGStockRptReader: TFGStockRptReader;
  i: Integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if IsNameHW(aFGStockRptReader.Items[j]^.snumber, aFGStockRptReader.Items[j]^.sname) then
        begin
          dOHOverSeaSF := dOHOverSeaSF + aFGStockRptReader.Items[j]^.dqty;
        end
        else
        begin
          dOHDomesticSF := dOHDomesticSF + aFGStockRptReader.Items[j]^.dqty;
        end;
      end;
    end;
  end;
...
procedure TfrmMakeFGReport2.btnSave2Click(Sender: TObject);
const
  CSProds: array[0..1] of string = ('魅族', '魅蓝');
var
  slWin: TStringList;
  slStock: TStringList;
  slUnOut: TStringList;
...
//  aFGDBRptReader: TFGDBRptReader;
  ...
  iino: Integer;
  bino: Boolean;
  dOHOverSea, dOHDomestic: Double;
  dOHOverSeaSF, dOHDomesticSF: Double;
  dOHOverSeaORT, dOHDomesticORT: Double;
  dOHOverSeaKJ, dOHDomesticKJ: Double;
begin
  sfile := CSProds[rgMZ.ItemIndex] + '成品报表' + FormatDateTime('yyyyMMdd', Now);
  ...
  if rgMZ.ItemIndex = 0 then
  begin
    slProjNo := slProjNo_OEM;
  end
  else
  begin
    slProjNo := slProjNo_ODM;
  end;
  ...
  try
    ...
    SaveWin(ExcelApp, aFGRptWinReader, slWin, slWinBS, slProjNo);
    ...
    SaveStock(ExcelApp, slStock, slStockBS, slDB, slDBBS, slProjNo);
    ...
    SaveUnOut(ExcelApp, slUnOut, slUnOutBS, slProjNo);
    ...
    Save_SF_KJ_ORT(ExcelApp, slSF, slProjNo); // 顺丰全部算魅族的
    ...
    Save_SF_KJ_ORT(ExcelApp, slKJ, slProjNo);
    ...
    Save_SF_KJ_ORT(ExcelApp, slORT, slProjNo);
    ...
    try              
      ...
      begin
        ...
        if slProjNo.IndexOfName( Copy(pn^.snumber, 1, 5) ) < 0 then
        begin
          Continue;
        end;
      ...
      ExcelApp.Cells[irow, 8].Value := '总库存';               
      ExcelApp.Cells[irow + 1, 8].Value := '国内';
      ExcelApp.Cells[irow + 1, 9].Value := '海外';
      ExcelApp.Cells[irow + 1, 10].Value := '合计';
      ...
      MergeCells(ExcelApp, irow, 8, irow, 10);
      ...
      ExcelApp.Columns[9].ColumnWidth := 12;
      ExcelApp.Columns[10].ColumnWidth := 12;

      ExcelApp.Rows[irow].RowHeight := 27;

      AddColor(ExcelApp, 1, 1, 2, 10, $DAC0CC);
      ...
      irow := irow + 2;
      irow1 := irow;
      for i := 0 to slProjNo.Count - 1 do
      begin
        ExcelApp.Cells[irow, 1].Value := slProjNo.ValueFromIndex[i];
        ExcelApp.Cells[irow, 2].Value := GetSumWin(slProjNo.Names[i], aFGRptWinReader, slWin, False);
        ExcelApp.Cells[irow, 3].Value := GetSumStockAlll(slProjNo.Names[i], slStock, slDB, dOHOverSea, dOHDomestic);           // C
        ExcelApp.Cells[irow, 4].Value := GetSumStockNoUnOut(slProjNo.Names[i], slStock, slDB, slUnOut);
        ExcelApp.Cells[irow, 5].Value := GetSumSF_KJ_ORT(slProjNo.Names[i], slSF, dOHOverSeaSF, dOHDomesticSF, True);          // E
        ExcelApp.Cells[irow, 6].Value := GetSumSF_KJ_ORT(slProjNo.Names[i], slKJ, dOHOverSeaKJ, dOHDomesticKJ, False);         // F
        ExcelApp.Cells[irow, 7].Value := GetSumSF_KJ_ORT(slProjNo.Names[i], slORT, dOHOverSeaORT, dOHDomesticORT, False);      // G
        ExcelApp.Cells[irow, 8].Value := dOHDomestic + dOHDomesticSF + dOHDomesticKJ + dOHDomesticORT;
        ExcelApp.Cells[irow, 9].Value := dOHOverSea + dOHOverSeaSF + dOHOverSeaKJ + dOHOverSeaORT;
        ExcelApp.Cells[irow, 10].Value := '=C' + IntToStr(irow) + '+E' + IntToStr(irow) + '+F' + IntToStr(irow) + '+G' + IntToStr(irow);
        irow := irow + 1;

                     

        sSumWin := GetSumWin(slProjNo.Names[i], aFGRptWinReader, slWinBS, True);
        sSumStockAll := GetSumStockAlll(slProjNo.Names[i], slStockBS, slDBBS, dOHOverSea, dOHDomestic);
        dSumStockNoUnOut := GetSumStockNoUnOut(slProjNo.Names[i], slStockBS, slDBBS, slUnOutBS);
        if (sSumWin > 0) or (sSumStockAll > 0) or (dSumStockNoUnOut > 0) then
        begin
          ExcelApp.Cells[irow, 1].Value := slProjNo.ValueFromIndex[i] + 'BS';
          ...
          ExcelApp.Cells[irow, 8].Value := dOHDomestic;
          ExcelApp.Cells[irow, 9].Value := dOHOverSea;
          ExcelApp.Cells[irow, 10].Value := '=C' + IntToStr(irow) + '+E' + IntToStr(irow) + '+F' + IntToStr(irow) + '+G' + IntToStr(irow);
          irow := irow + 1;
        end;
      end;
      ...
      ExcelApp.Cells[irow, 1].Value := CSProds[rgMZ.ItemIndex] + '产品合计';
      ...
      AddColor(ExcelApp, irow, 1, irow, 10, $00FFFF);

      
      AddHorizontalAlignment(ExcelApp, 1, 1, 2, 10, xlCenter);
      ExcelApp.Range[ExcelApp.Cells[1, 1], ExcelApp.Cells[2, 10]].Font.Bold  := True;

      ExcelApp.Range[ExcelApp.Cells[irow, 2], ExcelApp.Cells[irow, 10]].NumberFormatLocal := '_ * #,##0_ ;_ * -#,##0_ ;_ * ""-""??_ ;_ @_ ';

      ExcelApp.Range[ExcelApp.Cells[1, 1], ExcelApp.Cells[irow, 10]].Font.Size := 10;
      ...
      for i := 0 to slProjNo.Count - 1 do
      begin
        ExcelApp.Cells[irow, 2].Value := slProjNo.ValueFromIndex[i];
        ExcelApp.Cells[irow, 3].Value := GetSumStockNoUnOut(slProjNo.Names[i], slStock, slDB, slUnOut)
          + GetSumSF_KJ_ORT(slProjNo.Names[i], slKJ, dOHOverSeaKJ, dOHDomesticKJ, False)
          + GetSumSF_KJ_ORT(slProjNo.Names[i], slORT, dOHOverSeaORT, dOHDomesticORT, False);
        ExcelApp.Cells[irow, 4].Value := GetSumStockRework(slProjNo.Names[i], slStock, slDB);
        ExcelApp.Cells[irow, 5].Value := GetSumStockUncheck(slProjNo.Names[i], slStock, slDB);
        ExcelApp.Cells[irow, 6].Value := GetSumStockNoUnOut_zhouzuan(slProjNo.Names[i], slStock, slDB);
        ExcelApp.Cells[irow, 7].Value := '=C' + IntToStr(irow) + '-D' + IntToStr(irow) + '-E' + IntToStr(irow) + '-F' + IntToStr(irow); 
        
        irow := irow + 1;
 
        dSumStockNoUnOut := GetSumStockNoUnOut(slProjNo.Names[i], slStockBS, slDBBS, slUnOutBS);
        dRework := GetSumStockRework(slProjNo.Names[i], slStockBS, slDBBS);
        dUncheck := GetSumStockUncheck(slProjNo.Names[i], slStockBS, slDBBS);
        if (dSumStockNoUnOut > 0) or (dRework > 0) or (dUncheck > 0) then
        begin
          ExcelApp.Cells[irow, 2].Value := slProjNo.ValueFromIndex[i] + 'BS';
      ...
      AddBorder(ExcelApp, irow1, 2, irow2, 7);
...
procedure TfrmMakeFGReport2.btnWinsBSClick(Sender: TObject);
var
  sfile: string;
begin
  if not ExcelOpenDialogs(sfile) then Exit;
...
procedure TfrmMakeFGReport2.btnStocksBSClick(Sender: TObject);
var
  sfile: string;
begin
  if not ExcelOpenDialogs(sfile) then Exit;
...
procedure TfrmMakeFGReport2.btnUnOutBSClick(Sender: TObject);
var
  sfile: string;
begin
  if not ExcelOpenDialogs(sfile) then Exit;
...
procedure TfrmMakeFGReport2.btnDBBSClick(Sender: TObject);
var
  sfile: string;
begin
  if not ExcelOpenDialogs(sfile) then Exit;
...
procedure TfrmMakeFGReport2.btnFGRptWinClick(Sender: TObject);
var
  sfile: string;
begin
  if not ExcelOpenDialogs(sfile) then Exit;
...
procedure TfrmMakeFGReport2.Log(const s: string);
begin
  Memo1.Lines.Add(s);
end;
