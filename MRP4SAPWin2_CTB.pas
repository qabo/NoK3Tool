        ...
        begin
          ...
          begin        
            ...
            begin
              ...
              begin
                ...
                dQtyUnalloc := (aMRPUnitPtr^.dQty - aMRPUnitPtr^.dQtyStock) * (100 - iPer100) / 100; // 未分配数量
          ...
          begin 
            ...
            begin       
              ...
              begin
                aMRPUnitPtr_Dep^.dQtyStock := aSAPMrpAreaStockReader.AllocStock2(aMRPUnitPtr_Dep^.snumber, dQty, aMRPUnitPtr_Dep^.sMrpArea);   // 如果某个替代料库存全满足了，剩余数量为0， 自动会为下面的替代料分配0的库存和需求              
              end;
        ...
        begin
          ...
          begin       
            ...
            aMRPUnitPtr_Dep^.dQtyStock2 := aSAPMrpAreaStockReader.AllocStock_area(
              ...
              dQty, aMRPUnitPtr_Dep^.sMrpArea, slTransfer);   // 如果某个替代料库存全满足了，剩余数量为0， 自动会为下面的替代料分配0的库存和需求
...
//            aMRPUnitPtr_Dep^.dQty := aMRPUnitPtr_Dep^.dQty + dQty * iPer100 / 100;
  ...
  try
    ...
    try
      sline := 'ID'#9'父ID'#9'物料'#9'物料名称'#9'需求日期'#9'建议下单日期'#9'需求数量'#9'可用库存'#9'可用库存2'#9'OPO'#9'净需求'#9'替代组'#9'MRP控制者'#9'采购员'#9'MRP区域';
      sl.Add(sline);
    ...
    Memo1.Lines.Add('保存  调拨建议');

    WorkBook.Sheets.Add(after:=WorkBook.Sheets[iSheet]);
    iSheet := iSheet + 1;
    ExcelApp.Sheets[iSheet].Activate;
    ExcelApp.Sheets[iSheet].Name := '调拨建议';


    sl := TStringList.Create;
    try
      sline := '物料编码'#9'物料名称'#9'数量'#9'需求日期'#9'FROM'#9'TO';
      sl.Add(sline);
    ...
    sl := TStringList.Create;
    try
      sline := '采购凭证'#9'行号'#9'物料'#9'物料名称'#9'建议数量'#9'建议到料日期'#9'订单交期'#9'建议'#9'MRP Area'#9'Mrp Area No'#9'MC'#9'采购员';
      sl.Add(sline);
    ...
    sl := TStringList.Create;
    try
      sline := '物料'#9'物料名称'#9'总需求'#9'总库存'#9'总订单'#9'库存呆滞'#9'订单呆滞'#9'E/O'#9'17天需求'#9'28天需求'#9'60天需求'#9'MRP类型'#9'MRP区域'#9'MC'#9'采购员';
      sl.Add(sline);
