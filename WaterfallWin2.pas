...
//    function GetLineData(lst: TList; const skey: string): TLineData;
  ...
  private
    ...
    procedure WriteWF11(ExcelApp: Variant; sl, slMerge: TStringList; aProj: TProjData;
      bSOP: Boolean);
    procedure WriteWFKeys(ExcelApp: Variant; sl, slMerge: TStringList;
      aWeekData: TWeekData; const sTitle: string;
      iDateQtyCount: Integer; aLst: TList; aKeys: TStringList; var irow, irow1, irow2: Integer);
    procedure WriteWF01(ExcelApp: Variant; sl, slMerge: TStringList; aProj: TProjData);
    procedure WriteWF11Compares(ExcelApp: Variant; sl: TStringList;
      aProj: TProjData;
    ...
    procedure WriteWF11ComparesTTL(ExcelApp: Variant; sl: TStringList;
      aProj: TProjData;
      aColors: TStringList; const sKeyType: string; iDateQtyCount: Integer;
      var irow: Integer; aLineDatasGetter: TLineDatasGetter; bSOP: Boolean);
    ...
    procedure WriteWF11Sum(ExcelApp: Variant; sl, slMerge: TStringList;
      aProj: TProjData; bSOP: Boolean);
    procedure WriteWF01Sum(ExcelApp: Variant; sl, slMerge: TStringList;
      aProj: TProjData);
    procedure WriteWFKeysSum(ExcelApp: Variant; sl: TStringList; var sline: string;
      aWeekData: TWeekData; const sTitle: string;
      iMonthQtyCount: Integer; aLst: TList; aKeys: TStringList; var irow, irow1, irow2: Integer);
    procedure WriteWF11ComparesSum(ExcelApp: Variant; sl: TStringList;
      var sline: string; aProj: TProjData; 
...
class procedure TfrmWaterfall2.ShowForm;
var
  frmWaterfall: TfrmWaterfall2;
begin
  frmWaterfall := TfrmWaterfall2.Create(nil);
  try
...
procedure TfrmWaterfall2.ReadExcelFile(const sfile: string; aProc: TReadExcelProc);
var
...
procedure TfrmWaterfall2.WriteExcelFile(const sfile: string; aProc: TWriteExcelProc);
var
  ExcelApp, WorkBook: Variant;
begin
  try
...
procedure TfrmWaterfall2.btnWaterFallClick(Sender: TObject);
var
  sfile: string;
begin
...
procedure TfrmWaterfall2.btnSOPSumClick(Sender: TObject);
var
  sfile: string;
begin
  if not ExcelOpenDialog(sfile) then Exit;
...
procedure TfrmWaterfall2.tbSaveClick(Sender: TObject);
var 
  sfile: string;
  dwtick: DWORD;
begin
...
//  sfile := 'C:\Users\qiujinbo\Desktop\WF\wf.xlsx';
  ...
  try
    ...
    dwtick := GetTickCount - dwtick;
    Memo1.Lines.Add(Format('写取Waterfall耗时：%0.2f 秒', [dwtick / 1000]));
...
function TfrmWaterfall2.FindFirstNeedDateCellDetail(ExcelApp: Variant; var row, col: Integer): Boolean;
var
  r, c: Integer;
  s: string;
  s1, s2: string;
...
function TfrmWaterfall2.GetDateColRangeDetail(ExcelApp: Variant; r, c: Integer; var col1, col2: Integer): Boolean;
var
  v: Variant;
begin
  col1 := c;
...
procedure TfrmWaterfall2.GetWeekCountDetail(ExcelApp: Variant; irow, icol: Integer; lstRowOfWeeks: TList);
var
  r: Integer;
  s: string;
  s2: string;
...
function TfrmWaterfall2.ReadDatesQtyDetail(ExcelApp: Variant; irow: Integer;
  irow1, irow2: Integer; icol1, icol2: Integer; lst: TList; iKeyCol: Integer): string;
var
  r, c: Integer;
  dQty: Double;
...
procedure TfrmWaterfall2.ReadWeekDetail(ExcelApp: Variant; irow, icol: Integer;
  dtcol1, dtcol2: Integer; aProjData: TProjData);
var
  sWeek: string;
  irow1, irow2: Integer;
...
procedure TfrmWaterfall2.OpenWFDetail(ExcelApp, WorkBook: Variant; aProjData: TProjData);
var
  irow, icol: Integer;
...
procedure TfrmWaterfall2.OpenSOPSheet(ExcelApp: Variant; aProjData: TProjData;
  const sWeek: string);
var
  irow: Integer;
  irow1, irow2: Integer;
...
procedure TfrmWaterfall2.OpenWF(ExcelApp, WorkBook: Variant);
...
procedure TfrmWaterfall2.OpenSOP(ExcelApp, WorkBook: Variant);
var
  iSheetCount, iSheet: Integer;
  sSheet: string;
  sProj: string;
...
procedure TfrmWaterfall2.Clear;
var
  i: Integer;
  aProjData: TProjData;
begin
...
function TfrmWaterfall2.FindProj(const sProj: string): TProjData;
var
  i: Integer;
  aProjData: TProjData;
begin
...
procedure TfrmWaterfall2.SortDate;
var
  i: Integer;
  aProjData: TProjData;
begin
...
procedure TfrmWaterfall2.SumByMonth;
var
  i: Integer;
  aProjData: TProjData;
begin
...
procedure TfrmWaterfall2.WriteWFKeys(ExcelApp: Variant; sl, slMerge: TStringList;
  aWeekData: TWeekData; const sTitle: string;
  iDateQtyCount: Integer; aLst: TList; aKeys: TStringList; var irow, irow1, irow2: Integer);
var
  //icol: Integer;
...
//    ExcelApp.Cells[irow, 4].Value := '=SUM(' + GetRef(5) + IntToStr(irow) + ':' + GetRef(iDateQtyCount + 4) + IntToStr(irow) + ')';
    sline := sline + #9 + aLineData.FName + #9 + '=SUM(' + GetRef(5) + IntToStr(irow) + ':' + GetRef(iDateQtyCount + 4) + IntToStr(irow) + ')';
    for iDate := 0 to iDateQtyCount - 1 do
    begin
      //icol := iDate + 5;
      //ExcelApp.Cells[irow, icol].Value := aLineData.DateQtys[iDate]^.dQty;
      sline := sline + #9 + Format('%0.0f', [aLineData.DateQtys[iDate]^.dQty]);
    end;     
    irow := irow + 1;
    sl.Add(sline);
    sline := #9;
...
//  ExcelApp.Range[ExcelApp.Cells[irow1, 2], ExcelApp.Cells[irow2, 2]].MergeCells := True;
  slMerge.Add( Format('%d,%d,%d,%d', [irow1, 2, irow2, 2]) );
...
procedure TfrmWaterfall2.WriteWFKeysSum(ExcelApp: Variant; sl: TStringList;
  var sline: string; aWeekData: TWeekData; const sTitle: string;
  iMonthQtyCount: Integer; aLst: TList; aKeys: TStringList; var irow, irow1, irow2: Integer);
var
//  icol: Integer;
  iMonth: Integer;
...
begin
  ...
  begin
    ...
    sline := sline + #9 + aLineData.FName;
    
    for iMonth := 0 to iMonthQtyCount - 1 do
    begin
//      icol := iMonth + 4;
      //ExcelApp.Cells[irow, icol].Value := aLineData.MonthQtys[iMonth]^.dQty;
      sline := sline + #9 + Format('%0.0f', [aLineData.MonthQtys[iMonth]^.dQty]);
    end; 
    sline := sline + #9 + '=SUM(' + GetRef(4) + IntToStr(irow) + ':' + GetRef(iMonthQtyCount + 3) + IntToStr(irow) + ')';
...
procedure TfrmWaterfall2.WriteWF11Compares(ExcelApp: Variant; sl: TStringList;
  aProj: TProjData;
  aColors: TStringList; const sKeyType: string; iDateQtyCount: Integer;
  var irow: Integer; aLineDatasGetter: TLineDatasGetter; bSOP: Boolean);
var
...
//      ExcelApp.Cells[irow, 4].Value := '合计';
      sline := #9 + sKeyType + '变化(' + aColors[iKey] + ')' + '需求日期'#9'合计';

      for iDate := 0 to iDateQtyCount - 1 do
      begin
        icol := iDate + 5;
        //ExcelApp.Cells[irow, icol].Value := aProj.Weeks[0].Colors[0].DateQtys[iDate]^.dt;
        sline := sline + #9 + FormatDateTime('yyyy-MM-dd', aProj.Weeks[0].Colors[0].DateQtys[iDate]^.dt);
      end;
...
//        ExcelApp.Cells[irow, 4].Value := '=SUM(' + GetRef(5) + IntToStr(irow) + ':' + GetRef(iDateQtyCount + 4) + IntToStr(irow) + ')';
        sline := #9 + aWeekData.FName + #9 + aColors[iKey] + #9 + '=SUM(' + GetRef(5) + IntToStr(irow) + ':' + GetRef(iDateQtyCount + 4) + IntToStr(irow) + ')';
        for iDate := 0 to iDateQtyCount - 1 do
        begin
          icol := iDate + 5;
          ...
          begin
            ...
            begin
              ...
              sline := sline + #9 + '=0-' + GetRef(icol) + IntToStr(aLineData0.FRow);
            end;
          end
          else
          begin
            ...
            begin
              ...
              sline := sline + #9 + '=' + GetRef(icol) + IntToStr(aLineData.FRow) + '-' + GetRef(icol) + IntToStr(aLineData.FRow);
            end
            else
            begin
              //ExcelApp.Cells[irow, icol].Value :=  '=' + GetRef(icol) + IntToStr(aLineData.FRow) + '-' + GetRef(icol) + IntToStr(aLineData0.FRow);
              sline := sline + #9 + '=' + GetRef(icol) + IntToStr(aLineData.FRow) + '-' + GetRef(icol) + IntToStr(aLineData0.FRow);
            end;
...
//      ExcelApp.Cells[irow, 4].Value := '=SUM(' + GetRef(5) + IntToStr(irow) + ':' + GetRef(iDateQtyCount + 4) + IntToStr(irow) + ')';
      sline := #9#9 + aColors[iKey] + '累计变化' + #9 + '=SUM(' + GetRef(5) + IntToStr(irow) + ':' + GetRef(iDateQtyCount + 4) + IntToStr(irow) + ')';
      for iDate := 0 to iDateQtyCount - 1 do
      begin
        icol := iDate + 5;
//        ExcelApp.Cells[irow, icol].Value :=  '=SUM(' + GetRef(icol) + IntToStr(irow1) + ':' + GetRef(icol) + IntToStr(irow2) + ')';
        sline := sline + #9 + '=SUM(' + GetRef(icol) + IntToStr(irow1) + ':' + GetRef(icol) + IntToStr(irow2) + ')';
      end;                   
...
procedure TfrmWaterfall2.WriteWF11ComparesTTL(ExcelApp: Variant; sl: TStringList;
  aProj: TProjData;
  aColors: TStringList; const sKeyType: string; iDateQtyCount: Integer;
  var irow: Integer; aLineDatasGetter: TLineDatasGetter; bSOP: Boolean);
var
...
//  ExcelApp.Cells[irow, 3].Value := '需求日期';
//  ExcelApp.Cells[irow, 4].Value := '合计';
  sline := #9 + sKeyType + '变化' + #9 + '需求日期'#9'合计';
...
//      ExcelApp.Cells[irow, 4].Value := '=SUM(' + GetRef(5) + IntToStr(irow) + ':' + GetRef(iDateQtyCount + 4) + IntToStr(irow) + ')';
      sline := #9 + aWeekData.FName + #9 + sKeyType + #9 + '=SUM(' + GetRef(5) + IntToStr(irow) + ':' + GetRef(iDateQtyCount + 4) + IntToStr(irow) + ')';
      for iDate := 0 to iDateQtyCount - 1 do
      begin
        icol := iDate + 5;
...
//  ExcelApp.Cells[irow, 4].Value := '=SUM(' + GetRef(5) + IntToStr(irow) + ':' + GetRef(iDateQtyCount + 4) + IntToStr(irow) + ')';
  sline := #9#9 + sKeyType + '累计变化' + #9 + '=SUM(' + GetRef(5) + IntToStr(irow) + ':' + GetRef(iDateQtyCount + 4) + IntToStr(irow) + ')';
  for iDate := 0 to iDateQtyCount - 1 do
  begin
    icol := iDate + 5;
//    ExcelApp.Cells[irow, icol].Value :=  '=SUM(' + GetRef(icol) + IntToStr(irow1) + ':' + GetRef(icol) + IntToStr(irow2) + ')';
...
procedure TfrmWaterfall2.WriteWF11ComparesSum(ExcelApp: Variant; sl: TStringList;
  var sline: string; aProj: TProjData;
  aColors: TStringList; const sKeyType: string; iMonthQtyCount: Integer;
  var irow: Integer; aLineDatasGetter: TLineDatasGetter; wd1, wd2: TWeekData);
var
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          sline := sline + #9 + '=' + GetRef(icol) + IntToStr(aLineData0.FRow) + '-' + GetRef(icol) + IntToStr(aLineData0.FRow);
        end;
      end
      else
      begin
        ...
        begin
          ...
          sline := sline + '=' + GetRef(icol) + IntToStr(aLineData.FRow) + '-0';
        end
        else
        begin
          //ExcelApp.Cells[irow, icol].Value :=  '=' + GetRef(icol) + IntToStr(aLineData0.FRow) + '-' + GetRef(icol) + IntToStr(aLineData.FRow);
          sline := sline + #9 + '=' + GetRef(icol) + IntToStr(aLineData0.FRow) + '-' + GetRef(icol) + IntToStr(aLineData.FRow);
        end;
      end;
    end;
    //ExcelApp.Cells[irow, iMonthQtyCount + 4].Value := '=SUM(' + GetRef(4) + IntToStr(irow) + ':' + GetRef(iMonthQtyCount + 3) + IntToStr(irow) + ')';
...
procedure TfrmWaterfall2.WriteWF01Compares(ExcelApp: Variant; aProj: TProjData;
  aColors: TStringList; const sKeyType: string; iDateQtyCount: Integer;
  var irow: Integer; aLineDatasGetter: TLineDatasGetter);
var
  iKey: Integer;
...
procedure TfrmWaterfall2.WriteWF11Sum(ExcelApp: Variant; sl, slMerge: TStringList;
  aProj: TProjData; bSOP: Boolean);
var
  irow: Integer;
  icol: Integer;
  ...
  slineMonth, slineSum: string;
  slineMonthIdx, slineSumIdx: Integer;
  sline: string;
begin
  ...
  try
    ...
    begin
      ...
      slineMonthIdx := sl.Count ;
      sl.Add('');

      slineSumIdx := sl.Count;
      sl.Add('');

      //ExcelApp.Cells[irowMonth, 2].Value := 'S&OP日期';
      //ExcelApp.Cells[irowMonth, 3].Value := '月份';             
      slineMonth := #9 + 'S&OP日期' + #9 + '月份';
      
      //ExcelApp.Cells[irowSum, 3].Value := aProj.FName + ' S&OP';
      slineSum := #9#9 + aProj.FName + ' S&OP';

      //ExcelApp.Range[ ExcelApp.Cells[irowMonth, 2], ExcelApp.Cells[irowSum, 2] ].MergeCells := True;
      slMerge.Add( Format('%d,%d,%d,%d', [irowMonth, 2, irowSum, 2]) );

      irow := irow + 2;
      //ExcelApp.Cells[irow, 2].Value := aProj.Weeks[iWeek].FName;
      sline := #9 + aProj.Weeks[iWeek].FName;

      irow1 := irow;
      irow2 := irow;
                          
      irowVer1 := irow;                                                                              
      WriteWFKeysSum(ExcelApp, sl, sline, aProj.Weeks[iWeek], '制式', iMonthQtyCount, aProj.Weeks[iWeek].FVers, aVers, irow, irow1, irow2);
      irowVer2 := irow - 1;

      WriteWFKeysSum(ExcelApp, sl, sline, aProj.Weeks[iWeek], '容量', iMonthQtyCount, aProj.Weeks[iWeek].FCaps, aCaps, irow, irow1, irow2);
      WriteWFKeysSum(ExcelApp, sl, sline, aProj.Weeks[iWeek], '颜色', iMonthQtyCount, aProj.Weeks[iWeek].FColors, aColors, irow, irow1, irow2);
      WriteWFKeysSum(ExcelApp, sl, sline, aProj.Weeks[iWeek], '整机/裸机', iMonthQtyCount, aProj.Weeks[iWeek].FFGs, aFGs, irow, irow1, irow2);
      ...
      begin
        ...
        slineMonth := slineMonth + #9 + IntToStr( MonthOf( aProj.Weeks[0].Colors[0].MonthQtys[iMonth]^.dt ) ) + '月';
        ...
        slineSum := slineSum + #9 + '=IF(SUM(' + GetRef(icol) + IntToStr(irowVer1) + ':' + GetRef(icol) + IntToStr(irow2) + ')/4=SUM(' + GetRef(icol) + IntToStr(irowVer1) + ':' + GetRef(icol) + IntToStr(irowVer2) + '),SUM(' + GetRef(icol) + IntToStr(irowVer1) + ':' + GetRef(icol) + IntToStr(irowVer2) + '),"err")';
      end;
      //ExcelApp.Cells[irowMonth, iMonthQtyCount + 4].Value := 'TTL S&OP';
      slineMonth := slineMonth + #9 + 'TTL S&OP';

      //ExcelApp.Cells[irowSum, iMonthQtyCount + 4].Value := '=SUM(' + GetRef(4) + IntToStr(irowSum) + ':' + GetRef(iMonthQtyCount + 3) + IntToStr(irowSum) + ')';
      slineSum := slineSum + #9 + '=SUM(' + GetRef(4) + IntToStr(irowSum) + ':' + GetRef(iMonthQtyCount + 3) + IntToStr(irowSum) + ')';
      ...
      slMerge.Add( Format('%d,%d,%d,%d', [irowVer1, 2, irow2, 2]) );

      sl[slineMonthIdx] := slineMonth;
      sl[slineSumIdx] := slineSum;
    ...
    begin
      ...
      sline := #9 + aProj.FCurrWeekData.FName;

      irow1 := irow;
      irow2 := irow;
                          
      irowVer1 := irow;                                                                              
      WriteWFKeysSum(ExcelApp, sl, sline, aProj.FCurrWeekData, '制式', iMonthQtyCount, aProj.FCurrWeekData.FVers, aVers, irow, irow1, irow2);
      irowVer2 := irow - 1;

      WriteWFKeysSum(ExcelApp, sl, sline, aProj.FCurrWeekData, '容量', iMonthQtyCount, aProj.FCurrWeekData.FCaps, aCaps, irow, irow1, irow2);
      WriteWFKeysSum(ExcelApp, sl, sline, aProj.FCurrWeekData, '颜色', iMonthQtyCount, aProj.FCurrWeekData.FColors, aColors, irow, irow1, irow2);
      WriteWFKeysSum(ExcelApp, sl, sline, aProj.FCurrWeekData, '整机/裸机', iMonthQtyCount, aProj.FCurrWeekData.FFGs, aFGs, irow, irow1, irow2);
      ...
      begin
        ...
        slineMonth := slineMonth + #9 + IntToStr( MonthOf( aProj.FCurrWeekData.Colors[0].MonthQtys[iMonth]^.dt ) ) + '月';
    ...
    sline := #9 + sweek2 + ' vs ' + sweek1;

    irow1 := irow;
    WriteWF11ComparesSum(ExcelApp, sl, sline, aProj, aVers, '制式', iMonthQtyCount, irow, LineDatasGetter4Ver, wd1, wd2);
    WriteWF11ComparesSum(ExcelApp, sl, sline, aProj, aCaps, '容量', iMonthQtyCount, irow, LineDatasGetter4Cap, wd1, wd2);
    WriteWF11ComparesSum(ExcelApp, sl, sline, aProj, aColors, '颜色', iMonthQtyCount, irow, LineDatasGetter4Color, wd1, wd2);
    WriteWF11ComparesSum(ExcelApp, sl, sline, aProj, aFGs, '整机/裸机', iMonthQtyCount, irow, LineDatasGetter4FG, wd1, wd2);
    irow2 := irow - 1;
    
    //ExcelApp.Cells[irowMonth, 2].Value := 'S&OP变化';
    //ExcelApp.Cells[irowMonth, 3].Value := '月份';
    slineMonth := #9 + 'S&OP变化' + #9 + '月份';

    //ExcelApp.Cells[irowSum, 3].Value := aProj.FName + ' S&OP变化';
    slineSum := #9#9 + aProj.FName + ' S&OP变化';
    ...
    begin
      ...
      slineMonth := slineMonth + #9 + IntToStr( MonthOf( wd1.Colors[0].MonthQtys[iMonth]^.dt )) + '月';

      //ExcelApp.Cells[irowSum, iMonth + 4].Value := '=' + GetRef(iMonth + 4) + IntToStr(wd2.FRow) + '-' + GetRef(iMonth + 4) + IntToStr(wd1.FRow);
      slineSum := slineSum + #9 + '=' + GetRef(iMonth + 4) + IntToStr(wd2.FRow) + '-' + GetRef(iMonth + 4) + IntToStr(wd1.FRow);
    end;
...
procedure TfrmWaterfall2.WriteWF01Sum(ExcelApp: Variant; sl, slMerge: TStringList;
  aProj: TProjData);
var
  irow: Integer;
  icol: Integer;
...
//    slineSumIdx := sl.Count;
    ...
    begin
      ...
      //ExcelApp.Cells[irowSum, icol].Value := '=IF(SUM(' + GetRef(icol) + IntToStr(irowVer1) + ':' + GetRef(icol) + IntToStr(irow2) + ')/4=SUM(' + GetRef(icol) + IntToStr(irowVer1) + ':' + GetRef(icol) + IntToStr(irowVer2) + '),SUM(' + GetRef(icol) + IntToStr(irowVer1) + ':' + GetRef(icol) + IntToStr(irowVer2) + '),"err")';
      slineSum := slineSum + '=IF(SUM(' + GetRef(icol) + IntToStr(irowVer1) + ':' + GetRef(icol) + IntToStr(irow2) + ')/4=SUM(' + GetRef(icol) + IntToStr(irowVer1) + ':' + GetRef(icol) + IntToStr(irowVer2) + '),SUM(' + GetRef(icol) + IntToStr(irowVer1) + ':' + GetRef(icol) + IntToStr(irowVer2) + '),"err")';
    end;
    //ExcelApp.Cells[irowMonth, iMonthQtyCount + 4].Value := 'TTL S&OP';
    slineMonth := slineMonth + #9 + 'TTL S&OP';
...
procedure TfrmWaterfall2.WriteWF11(ExcelApp: Variant; sl, slMerge: TStringList;
  aProj: TProjData; bSOP: Boolean);
var
  irow: Integer;
  icol: Integer;
...
begin
  ...
  try
    ...
    for iDate := 0 to iDateQtyCount - 1 do
    begin
      sline := sline + #9 + 'WK' + IntToStr(WeekOf(aProj.Weeks[0].Colors[0].DateQtys[iDate]^.dt));
...
//      ExcelApp.Cells[irow, 3].Value := '需求日期';
//      ExcelApp.Cells[irow, 4].Value := '合计';
      sline := #9 + aProj.Weeks[iWeek].FName + #9'需求日期'#9'合计';
...
//        ExcelApp.Cells[irow, icol].Value := aProj.Weeks[0].Colors[0].DateQtys[iDate]^.dt;
      ...
      WriteWFKeys(ExcelApp, sl, slMerge, aProj.Weeks[iWeek], '颜色', iDateQtyCount, aProj.Weeks[iWeek].FColors, aColors, irow, irow1, irow2);
      WriteWFKeys(ExcelApp, sl, slMerge, aProj.Weeks[iWeek], '制式', iDateQtyCount, aProj.Weeks[iWeek].FVers, aVers, irow, irow1, irow2);
      WriteWFKeys(ExcelApp, sl, slMerge, aProj.Weeks[iWeek], '容量', iDateQtyCount, aProj.Weeks[iWeek].FCaps, aCaps, irow, irow1, irow2);
      WriteWFKeys(ExcelApp, sl, slMerge, aProj.Weeks[iWeek], '整机/裸机', iDateQtyCount, aProj.Weeks[iWeek].FFGs, aFGs, irow, irow1, irow2);
      WriteWFKeys(ExcelApp, sl, slMerge, aProj.Weeks[iWeek], '包装', iDateQtyCount, aProj.Weeks[iWeek].FPkgs, aPkgs, irow, irow1, irow2);

      sline := #9#9'MPS'#9'=SUM(' + GetRef(5) + IntToStr(irow) + ':' + GetRef(iDateQtyCount + 4) + IntToStr(irow) + ')';
...
//      ExcelApp.Cells[irow, 3].Value := '需求日期';
//      ExcelApp.Cells[irow, 4].Value := '合计';
      sline := #9 + aProj.FCurrWeekData.FName + #9'需求日期'#9'合计';
      ...
      WriteWFKeys(ExcelApp, sl, slMerge, aProj.FCurrWeekData, '颜色', iDateQtyCount, aProj.FCurrWeekData.FColors, aColors, irow, irow1, irow2);
      WriteWFKeys(ExcelApp, sl, slMerge, aProj.FCurrWeekData, '制式', iDateQtyCount, aProj.FCurrWeekData.FVers, aVers, irow, irow1, irow2);
      WriteWFKeys(ExcelApp, sl, slMerge, aProj.FCurrWeekData, '容量', iDateQtyCount, aProj.FCurrWeekData.FCaps, aCaps, irow, irow1, irow2);
      WriteWFKeys(ExcelApp, sl, slMerge, aProj.FCurrWeekData, '整机/裸机', iDateQtyCount, aProj.FCurrWeekData.FFGs, aFGs, irow, irow1, irow2);
      WriteWFKeys(ExcelApp, sl, slMerge, aProj.FCurrWeekData, '包装', iDateQtyCount, aProj.FCurrWeekData.FPkgs, aPkgs, irow, irow1, irow2);
...
//      ExcelApp.Cells[irow, iDate + 5].Value := 'WK' + IntToStr(WeekOf(aProj.Weeks[0].Colors[0].DateQtys[iDate]^.dt));
    ...
    WriteWF11Compares(ExcelApp, sl, aProj, aColors, '颜色', iDateQtyCount, irow, LineDatasGetter4Color, bSOP);
    WriteWF11Compares(ExcelApp, sl, aProj, aVers, '制式', iDateQtyCount, irow, LineDatasGetter4Ver, bSOP);
    WriteWF11Compares(ExcelApp, sl, aProj, aCaps, '容量', iDateQtyCount, irow, LineDatasGetter4Cap, bSOP);
    WriteWF11Compares(ExcelApp, sl, aProj, aFGs, '整机/裸机', iDateQtyCount, irow, LineDatasGetter4FG, bSOP);
    WriteWF11Compares(ExcelApp, sl, aProj, aPkgs, '包装', iDateQtyCount, irow, LineDatasGetter4Pkg, bSOP);
      
    WriteWF11ComparesTTL(ExcelApp, sl, aProj, aColors, '总量', iDateQtyCount, irow, LineDatasGetter4Color, bSOP);
...
procedure TfrmWaterfall2.WriteWF01(ExcelApp: Variant; sl, slMerge: TStringList;
  aProj: TProjData);
var
  irow: Integer;
  icol: Integer;
...
function TfrmWaterfall2.WriteWF(ExcelApp, WorkBook: Variant): Boolean;
var
  iProj: Integer;
  aProj: TProjData;
  iSheet: Integer;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          WriteWF11(ExcelApp, sl, slMerge, aProj, True);
        end
        else     //无 SOP
        begin
          WriteWF11(ExcelApp, sl, slMerge, aProj, False);
        end;
      ...
      begin
        if aProj.FCurrWeekData.ColorCount > 0 then  // 有 SOP
        begin
          WriteWF01(ExcelApp, sl, slMerge, aProj);
        end;
      end;
      ...
      for icell := 0 to slMerge.Count - 1 do
      begin
        slcell.Text := StringReplace(slMerge[icell], ',', #13#10, [rfReplaceAll]);
        MergeCells(ExcelApp, StrToInt(slcell[0]), StrToInt(slcell[1]),
          StrToInt(slcell[2]), StrToInt(slcell[3]));
      end;
      ...
      begin
        if aProj.FCurrWeekData.ColorCount > 0 then  // 有 SOP
        begin
          WriteWF11Sum(ExcelApp, sl, slMerge, aProj, True);
        end
        else     //无 SOP
        begin
          WriteWF11Sum(ExcelApp, sl, slMerge, aProj, False);
        end;
      end    // 无 WF ， 有Proj， 肯定有SOP
      else
      begin
        if aProj.FCurrWeekData.ColorCount > 0 then  // 有 SOP
        begin
          WriteWF01Sum(ExcelApp, sl, slMerge, aProj);
        end;
      end;
...
procedure TfrmWaterfall2.FormCreate(Sender: TObject);
var
  ini: TIniFile;
begin
...
procedure TfrmWaterfall2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TfrmWaterfall2.FormDestroy(Sender: TObject);
var
  ini: TIniFile;
begin
  ini := TIniFile.Create(AppIni);
